...
func InitializeProtocol(pid int, configFolder string) (relativeProt *ProtocolInfo) {
	...
	//log.LLvl1(pid, "Saving the cryptoParams to files ***********")

	log.LLvl1(pid, "Reading the cryptoParams from files ***********")

	//if pid == 0 {
	//	cps = localCryptoParams[pid]
	//} else {
	//	cps = localCryptoParams[pid-1]
	//}

	//crypto.SaveCryptoParamsAndRotKeys(pid, cps.Sk, cps.AggregateSk, cps.Pk, cps.Rlk, cps.RotKs)
	cps := crypto.NewCryptoParamsFromDisk(pid == 0, pid, 2)
