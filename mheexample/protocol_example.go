...
func (pi *ExampleProtocolInfo) ExampleProtocol() {
	...
	if pid > 0 {
		...
		if pid == 2 {
			gwas.SaveFloatVectorToFile("test", dummyVector)
		...
		if pid == 1 {
			...
			matrixRead, _ := crypto.LoadCipherMatrixFromFile(cps, "decryptedRes")
			log.LLvl1("decryption ", crypto.DecryptFloatVector(cps, matrixRead[0], 10))
		} else if pid == 2 {
			CollectiveDecryptClientReceive(pid, "test_decryption", "decrypted", "decryptedRes_2")
			matrixReadNew, _ := crypto.LoadCipherMatrixFromFile(cps, "decryptedRes_2")
			log.LLvl1("decryption ", crypto.DecryptFloatVector(cps, matrixReadNew[0], 10))
		}
...
func ClientEncryptVector(pid int, vectorfileName string, vectorSize int) {
	...
	vectorToAggr := gwas.LoadFloatVectorFromFile(vectorfileName, vectorSize)
...
func CollectiveBootstrapClientSend(pid int, vectorToBootstrapFile string, out_path string, nbrOfClients int) {
	...
	sizes, bytes := mpc.MarshalPolyMat(refSharesDecrypt)
	bs := make([]byte, 4)
	binary.LittleEndian.PutUint32(bs, uint32(levelStart))
	crypto.WriteFullFile(out_path+"_"+strconv.Itoa(pid)+"_level", bs)
	crypto.WriteFullFile(out_path+"_"+strconv.Itoa(pid)+"_sizes", sizes)
	crypto.WriteFullFile(out_path+"_"+strconv.Itoa(pid), bytes)
}
...
func CollectiveBootstrapClientReceive(pid int, vectorToBootstrapFile string, crpsPath string, server_polysPath string, out_path string) {
	...
	// TODO this probably needs to be made deterministic
	refProtocol := dckks.NewRefreshProtocol(parameters)
	for i := range cm {
		for j := range cm[i] {
			//no communication
...
func CollectiveBootstrapServer(vectorsToBootstrap string, out_path string) {
	...
	var levels []int
	for {
		readsizesBytes, err := crypto.LoadFullFile(vectorsToBootstrap + "_" + strconv.Itoa(fileIndex) + "_sizes")
		if err != nil {
			break
		}
		readPolysBytes, err := crypto.LoadFullFile(vectorsToBootstrap + "_" + strconv.Itoa(fileIndex))
		readLevelsBytes, err := crypto.LoadFullFile(vectorsToBootstrap + "_" + strconv.Itoa(fileIndex) + "_level")
		levels = append(levels, int(binary.LittleEndian.Uint32(readLevelsBytes)))
		polyData := mpc.UnmarshalPolyMat(readsizesBytes, readPolysBytes)
		polysToBootstrap = append(polysToBootstrap, polyData)
		//TODO do better if changes smth
		polyData2 := mpc.UnmarshalPolyMat(readsizesBytes, readPolysBytes)
...
func CollectiveDecryptClientReceive(pid int, vectorToDecryptFile string, server_polysPath string, out_path string) {
	...
	gwas.SaveFloatVectorToFile(out_path, pmDecoded)
}
